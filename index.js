...
app.get("/contributor", async (req, res) => {
    const controller = require('./src/contributor/contributorController');
    let item = await controller.getList();
    res.json(item).end();
});
app.post("/contributor", async (req, res) => {
    const controller = require('./src/contributor/contributorController');
    let item = await controller.parsePost(req.body);
    res.json(item).end();
});
app.delete("/contributor", async (req, res) => {
    const controller = require('./src/contributor/contributorController');
    let item = await controller.deleteItem(req.body);
    res.json(item).end();
});
